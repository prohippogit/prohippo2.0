...
exports.build = function build({ REGIONS, PRIMARY_REGION, TRIGGER_REGION, db, recordSpend, STORAGE_BUCKET }) {
  ...
  /* ---------------- 3. the weekly cause list ----------------
   *
   * Saturday 07:30 IST. The file lands before the weekend, while there is still
   * a working day left to fix a clash — Sunday evening would be too late to ring
   * a bench clerk.
   *
   * THE PDF IS DRAWN HERE, BY THE SAME RENDERER THE BROWSER USES. Nobody has a
   * browser open at half past seven on a Saturday, so unlike the invoice this
   * one cannot be generated by the client and uploaded. `functions/shared/` is a
   * copy of src/'s renderer made at deploy time by
   * scripts/build-functions-shared.mjs — a copy rather than a second
   * implementation, so the sheet that arrives on WhatsApp is the sheet the
   * Hearings page prints, down to the day bands.
   *
   * The dynamic import is how CommonJS reaches ESM: functions/ is CommonJS and
   * the renderer is ESM, which Node 22 bridges with `await import()`. It also
   * means a practice with no hearings never loads 166 KB of fonts.
   */
  async function renderCauseList({ hearings, from, to, profile }) {
    let buildCauseListPDF;
    try {
      ({ buildCauseListPDF } = await import("./shared/causeListPdf.js"));
    } catch (e) {
      /* The one failure mode of the copy step, named plainly. functions/shared
         is git-ignored and built by a predeploy hook, so a deploy run some other
         way — or a hook that was removed — arrives without it, and Node's own
         message says only that a path does not exist. */
      throw new Error(
        `The PDF renderer is missing from functions/shared (${e.message || e}). ` +
        `It is copied from src/ by scripts/build-functions-shared.mjs, which firebase.json ` +
        `runs as a predeploy hook — run it and deploy again.`,
        { cause: e }
      );
    }
    const doc = buildCauseListPDF({ hearings, from, to, profile: profile || {} });
  ...
  async function publishCauseList(uid, from, buffer) {
    ...
    try {
      ...
      /* Almost always the one missing IAM grant. Said plainly, because the raw
  ...
  const whatsappWeeklyCauseList = onSchedule(
    {
    ...
    async () => {
      ...
      const { from, to } = nextWeekRange(istDate());
      let sent = 0;
      let quiet = 0;
      ...
      for (const userDoc of users.docs) {
        const uid = userDoc.id;
        const profile = userDoc.data() || {};
        if (!whatsAppEnabledFor(profile, "causeList")) continue;
        const reach = userReachability(profile);
        if (!reach.ok) continue;

        try {
          const snap = await db.collection(`users/${uid}/hearings`)
            .where("date", ">=", from).where("date", "<=", to).get();
          const hearings = snap.docs.map((d) => d.data()).filter(isHearingLive);

          /* An empty week is not sent. A weekly message that usually says
             "nothing listed" is the one people mute, and muting it costs them
             the weeks that did have something. */
          if (!hearings.length) { quiet += 1; continue; }

          // Claimed before the render, not after: the PDF costs real time and
          // memory, and a redelivered schedule should not pay for it twice.
          ...
          await deliver({
            uid,
            to: reach.mobile,
            template: "ph_cause_list_weekly_v1",
            params: causeListParams(hearings, { from, to }),
            media: { url, fileName: causeListFileName({ from, to }) },
            recipientRole: "user",
            subject: `Cause list — ${weekLabel(from, to)}`,
            body: `${hearings.length} hearings`,
          });
          sent += 1;
        } catch (e) {
          // One practice's failure must not end the sweep for everybody else's.
          console.error("cause list failed for", uid.slice(0, 8), e.message || e);
        }
      }

      console.log(`weekly cause list: ${sent} sent, ${quiet} practices with an empty week (${from} to ${to})`);
    }
  );
