 ...
 * the fast one.
 *
 * There are two copies of this file — this one for the web app’s ESM, and
 * connector/src/main/syncScope.js for the connector’s CommonJS main process —
 * because neither can import the other. test/syncScope.test.mjs runs both over
...
export const SCOPES = ["eproc", "all", "appeals", "returns", "returnForm"];

// What a run fetches when nobody said otherwise.
export const DEFAULT_SCOPE = "eproc";
...
export function isFirstSync(lastSyncedAt) {
 ...
 * exists to prevent. */
export function scopeForAssessee(requested, lastSyncedAt) {
